...
import ProductShimmer from '../categoryDetails/ProductShimmer';
...
const ViewAllCat = ({
...
}: RootStakeScreenProps<'ViewAllCat'>) => {
  ...
  useEffect(() => {
    if (id === '680b340bad130901da811111') {
      setName('Super Deals');
    } else if (id === '680b340bad130901da822222') {
      setName('Trends');
    }
  }, [id]);
  ...
  const isCategoryLoading = loading || !productsByCategory[id];
   const toggleLoved = async (productId: string, isLoved: boolean) => {
      ...
      console.log(`Optimistic update: ${isLoved ? 'Removing' : 'Adding'} ${productId}`);
  ...
  return (
    <SafeAreaView className="bg-white h-full">
      ...
      <View className="flex-row justify-between gap-1 m-2">
        <View className="bg-slate-200 h-10 mt-1 justify-center items-center w-[49%]">
          <Text className="font-semibold">{productData.length} items</Text>
        </View>
        <TouchableOpacity
          className="bg-slate-200 h-10 mt-1 justify-center items-center w-[49%]"
